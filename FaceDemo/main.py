...
def load_data(dataset_path):
    ...
    #Defeine a training sample of 20*20,total of 40,10 sample pictures each
    img_ndarray=np.asarray(img,dtype='float64')/256
    #img_ndarray = np.asarray(img, dtype='float64') / 32

    #record matrix of facial data
    ...
    #Divide the data into Training Set,Verifying Set and Testing Set
...
def convolutional_neural_network(data):
    ...
    with tf.variable_scope("conv_layer2") as layer2:
        layer2_output=convolutional_layer(
            data=layer1_output,kernel_size=kernel_shape2,
            bias_size=biase_shape2,
            pooling_size=[1,2,2,1]
        )

    with tf.variable_scope("full_connection") as layer3:
